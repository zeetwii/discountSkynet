...
class CameraAI:
    ...
    def __init__(self, cameraSelect=0):
        ...
        # model stuff
        self.model = YOLO("./models/yolo11n.pt", verbose=False)
    ...
    def comptuerVisionThread(self):
        ...
        while True:
            ...
            description = f"There are {str(len(self.detectedObjects))} objects detected in frame by the camera you are connected to.  The individual breakdown of all objects dected are in this list: {str(self.detectedObjects)}"

            #self.channel.basic_publish(exchange='', routing_key='cameraOutput', body=str(self.detectedObjects))
            self.channel.basic_publish(exchange='', routing_key='cameraOutput', body=str(description))
            ...
            #print("")
            time.sleep(5) # done to slow things down so as not to bog down the rest of the system.
